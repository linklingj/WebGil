...
## 10. 측정 (2026-08-18, jsdom 경로 · OpenAI `gpt-5.6-luna`)

`pnpm --filter @webgil/bench refine`, 사이트 13개. 실제 LLM 호출 결과:

| 페이지 | 최상위 | 노드 | 삭제 | 시간 | 불변식 |
|---|---|---|---|---|---|
| 데이터자격검정 | 3 → **6** | 60 → 60 | 8 | 18.1s | ✓ |
| 기상청 특보 | 14 → **4** | 262 → 186 | 79 | 14.6s | ✓ |
| 서울열린데이터 | 3 → **4** | 233 → 154 | 92 | 24.5s | ✓ |
| 안전신문고 | 4 → 4 | 72 → 72 | 0 | 7.6s | ✓ (무변화) |
| 안전운전 통합민원 | 6 → **4** | 216 → 180 | 54 | 29.7s | ✓ |
| 야놀자 | 6 → **4** | 221 → 193 | 35 | 21.5s | ✓ |
| 네이버 항공권 | 6 → **3** | 102 → 99 | 15 | 16.5s | ✓ |
| 허깅페이스 | 10 → **3** | 222 → 242 | 5 | 33.9s | ✓ |
| 세종대 공지 | — | — | — | — | 한도 초과 → 건너뜀 |
| 오픈소스대회·홈택스×2·무신사 | — | — | — | — | 노드 0(JS 렌더) → 건너뜀 |

**불변식은 8/8 전부 통과** — 핸들 소실 0, 텍스트 변조 0, 날조 id 0. §2의 핵심 제약(핸들·텍스트는 원본에서만 온다)이 실제 LLM 출력에 대고도 지켜진다. 벤치가 매 실행마다 이 세 값을 찍는다.

목표였던 P1-E(최상위가 chrome으로 채워짐)는 해결된다. 기상청은 `본문 바로가기 | 머리말 | 옵션 메뉴 | 주 메뉴 | 보조 정보 | 부 메뉴 | 페이지 경로 | 바닥글 | 탐색 ×6` → `페이지 내용 | 사용자 설정 | 사이트 머리말 | 바닥글`.

삭제량(최대 39%)을 확인한 결과 **본문 손실이 아니라 중복 제거**였다. 서울열린데이터에서 지운 링크 81개는 데스크톱·모바일용으로 두 번 렌더되는 메가메뉴의 사본이고, 전역 내비게이션 자체는 `탐색` 그룹 아래에 그대로 남는다. 스크린리더에서 사이트 이동 수단이 사라지면 개선이 아니라 퇴행이므로, 삭제 비율이 크게 나오는 페이지는 이 확인을 반드시 거쳐야 한다.

한계 두 가지가 같이 보였다:

- **무변화 사례**: 안전신문고는 모델이 입력을 그대로 돌려줘 재구성이 일어나지 않았다(삭제 0, 최상위 그대로). 실패는 아니지만 이득도 없다.
- **지연 7.6~33.9초**: 매 mutation 자동 실행이 불가능한 수준. §11의 "명시 트리거로만 실행"이 선택이 아니라 필수임을 확인.
