...
패널에 포커스가 있을 때는 맨손 방향키로 탐색한다 — 화면에 보이는 방향 그대로 `←`/`→`는 옆 형제, `↓`는 하위, `↑`는 상위다. `Home`으로 카메라를 현재 위치로 되돌린다(직접 팬·줌하면 자동 추적이 멈추고 "현재 위치로" 버튼이 나타난다).

포커스가 페이지에 있어도 되는 브라우저 단축키:
...
다른 확장과 겹치면 `chrome://extensions/shortcuts`에서 바꿀 수 있다(겹치면 Chrome이 조용히 등록을 건너뛴다).
...
1. [Ollama](https://ollama.com)를 설치하고 모델을 받는다: `ollama pull llama3.2`
2. 서버를 켠다: `ollama serve` (앱을 실행 중이면 이미 켜져 있다)
3. 패널 설정 → 제공자 **Ollama** → 설치된 모델 목록에서 고르고 저장
